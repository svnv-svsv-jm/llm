...
def test_pop_params_not_in_fn() -> None:
    ...
    inputs, popped = pop_params_not_in_fn(_input_fn, params)
    logger.info(inputs)
    assert "arg1" in inputs
    assert "arg2" in inputs
    assert "will_be_popped" not in inputs
    assert "will_be_popped" in popped
